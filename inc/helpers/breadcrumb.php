...
if (!function_exists('cct_breadcrumb')) {
	function cct_breadcrumb() {
		$html = '<ul class="cct-breadcrumbs d-flex">' . join("", cct_get_breadcrumb_items()) . '</ul>';
...
if (!function_exists('cct_get_breadcrumb_items')) {
	function cct_get_breadcrumb_items()
	{
		...
		/* Link to front page. */
		if (!is_front_page()) {
            $item[] = '<li><a href="' . esc_url(home_url('/')) . '" class="home">' . esc_html__('Home', 'cct') . '</a></li>&nbsp/&nbsp</i></i>';
		}
